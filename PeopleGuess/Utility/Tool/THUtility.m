...
+ (uint8_t *)base64BufferwithData:(NSData *)objData length:(int*)length
{
    if (length == NULL)
    {
        return NULL;
...
+ (uint8_t *)bufferWithBase64Data:(NSData *)objData length:(int*)length
{
    if (length == NULL)
    {
        return NULL;
...
+ (NSString *)encodeBase64AtURL:(NSURL *)fileUrl
{
    ...
    int size = [fileHandle seekToEndOfFile];
...
+ (BOOL)encodeBase64AtURL:(NSURL *)srcURL toURL:(NSURL *)dstURL
{
    ...
    int size = [srcHandle seekToEndOfFile];
